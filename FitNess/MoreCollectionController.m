...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(requestCollectionData) name:@"Food_Collection" object:nil];
    }
    return self;
}
...
- (void)selectbuttonAction:(UIButton *)sender
{
    ...
    self.selectedIndex = sender.tag - 66;
...
- (void)requestCollectionData
{
    ...
    [[NetworkManager shareManager] requestParams:parametersDic withModule:MODELS_MODEL_CHECK_MODULE2 method:MODELS_METHOD_GET_MY_FAVORITE finishBlock:^(DataServies *request, id result) {
        ...
        weakSelf.collectionArray = [NSMutableArray arrayWithObjects:[NSMutableArray array],[NSMutableArray array],[NSMutableArray array], nil];
        ...
        if (weakSelf.collectionArray.count > 0) {
            weakSelf.selectedArray = weakSelf.collectionArray[weakSelf.selectedIndex];
        }
