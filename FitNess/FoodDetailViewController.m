...
- (void)changeFoodDetailViewWithComment:(NSString *)comment effect:(NSString *)effect
{
    ...
    if ([self.typeId integerValue] == 2) {
        ...
        //http+:[^\\s]* 这是检测网址的正则表达式
            ...
            BOOL isFinished = YES;
            NSRange range0 =  [comment rangeOfString:[NSString stringWithFormat:@"%d.",index]];
            NSRange range1 =  [comment rangeOfString:[NSString stringWithFormat:@"%d。",index]];
            ...
            if (range0.location != NSNotFound || range1.location != NSNotFound) {
                ...
                while (isFinished) {
                    ...
                    if (range.location != NSNotFound) {
                        ...
                        if (regex != nil) {
                            NSString *tempStr = [[comment substringFromIndex:range.location] stringByReplacingOccurrencesOfString:@" " withString:@""];
    ...
    }else if([self.typeId integerValue] == 1){
        //组装一个字符串，把里面的网址解析出来
        NSError *error;
        //http+:[^\\s]* 这是检测网址的正则表达式
        NSInteger index = 1;
        NSMutableString *str = [NSMutableString string];
        effect  = [effect stringByReplacingOccurrencesOfString:@" " withString:@""];
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"\\d\\.[\u4E00-\u9FA5|\\w|\\,]*" options:NSRegularExpressionUseUnicodeWordBoundaries error:&error];
        NSRange range =  [effect rangeOfString:[NSString stringWithFormat:@"%d.",index]];
        if (range.location != NSNotFound) {
            NSArray *array = [regex matchesInString:effect options:0 range:NSMakeRange(0, [effect length])];
            NSString *str1 = nil;
            for (NSTextCheckingResult* b in array)
                
            {
//                str1 是每个和表达式匹配好的字符串。
                str1 = [effect substringWithRange:b.range];
                NSLog(@" str 1 is %@",str1);
               [str appendString:[NSString stringWithFormat:@"%@\n",str1]];
            }
            
             self.foodEffectLabel.text = str;
        }else
        {
       ...
       self.foodCommentLabel.text = comment;
        ...
        NSRange range0 =  [effect rangeOfString:[NSString stringWithFormat:@"%d.",1]];
        NSRange range1 =  [effect rangeOfString:[NSString stringWithFormat:@"%d。",1]];
        
        if (range0.location != NSNotFound || range1.location != NSNotFound) {
            self.foodEffectLabel.text = [self stringFormatWith:effect];
        }else{
...
- (NSString *)stringFormatWith:(NSString *)strOrigin
{
    //组装一个字符串，把里面的网址解析出来
    NSError *error;
    //http+:[^\\s]* 这是检测网址的正则表达式
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"\\d\\.[\u4E00-\u9FA5|\\w|\\,]*" options:NSRegularExpressionUseUnicodeWordBoundaries error:&error];
    NSRange range =  [strOrigin rangeOfString:[NSString stringWithFormat:@"%d.",index]];
    if (range.location != NSNotFound) {
        ...
        {
            //                str1 是每个和表达式匹配好的字符串。
            str1 = [strOrigin substringWithRange:b.range];
            NSLog(@" str 1 is %@",str1);
            [str appendString:[NSString stringWithFormat:@"%@\n",str1]];
        }
    }
