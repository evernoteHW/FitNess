...
@interface StatusCommentViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,EGORefreshTableHeaderDelegate,UITextViewDelegate>
{
...
- (void)viewDidLoad
{
    ...
    [rbutton setTitle:@"举报" forState:UIControlStateNormal];
...
- (UIView *)commentsTableHeadView
{
    if (_commentsTableHeadView == nil) {
        ...
        self.contentLabel.height = self.contentLabel.optimumSize.height + 13;
//        [_commentsTableHeadView addSubview:self.contentLabel];
        
        self.contentTextView = [[UITextView alloc]initWithFrame:CGRectMake(_px(120),self.headImageBtn.bottom, _px(466), _px(2))];
        self.contentTextView.font = FONT_SIZE(_px(24));
        self.contentTextView.textColor =[UIColor grayColor];
        self.contentTextView.backgroundColor = [UIColor clearColor];
        self.contentTextView.text = self.selectedModel.userMessageVO.msgContent;
//        self.contentTextView.delegate = self;
        self.contentTextView.editable = NO;
        self.contentTextView.height = self.contentLabel.optimumSize.height + 13;
        [_commentsTableHeadView addSubview:self.contentTextView];
...
- (void)requstMsgData
{
    ...
    [[NetworkManager shareManager] requestParams:parametersDic withModule:MODELS_MODEL_PARTYMANAGER method:MODELS_METHOD_QRY_USER_COMMENT_LIST finishBlock:^(DataServies *request, id result) {
        ...
        weakSelf.commentsCountLabel.text = [NSString stringWithFormat:@"       共有%d条评论",weakSelf.commentsArray.count];
        [weakSelf hideHud];
...
{
    NSLog(@"#pragma mark 点击图片放大");
    // 1.封装图片数据
    // 替换为中等尺寸图片
    NSString *url = self.selectedModel.userMessageVO.msgImg;
    YTPhoto *photo = [[YTPhoto alloc] init];
    photo.url = [NSURL URLWithString:url]; // 图片路径
    photo.srcImageView = self.imgContView; // 来源于哪个UIImageView
